...
def build_product_lookup(csv_path: str, category: str) -> dict:
    ...
    """
    print(f"Step 1: Building product lookup for {category}...")
    
    # Get unique product ASINs from the CSV
    print("Reading CSV to get unique products...")
    ...
    print(f"Found {len(unique_asins)} unique products to lookup")
    
    if len(unique_asins) == 0:
        print("❌ No ASINs found in CSV")
        return {}
    
    # Build lightweight lookup from metadata
    print("Building lookup from metadata...")
    ...
    try:
        ...
        for item in metadata_dataset:
            ...
            # Check if this product is in our CSV
        ...
        print(f"✓ Built lookup with {len(product_lookup)} products from {processed_count:,} metadata items")
        
        if len(product_lookup) == 0:
            print("⚠️  No products found in metadata")
        else:
...
def add_columns_fast(csv_path: str, product_lookup: dict) -> bool:
    ...
    try:
        ...
        if 'user_id' in df.columns:
            ...
            print("⚠️  user_id column not found, setting reviewer_review_count to NaN")
        ...
        print(f"✅ Added rating_vs_product_avg_abs and reviewer_review_count columns")
...
def main():
    ...
    print(f"🚀 Processing {args.csv_path} with lightweight lookup approach")
    print(f"Category: {args.category}")
    print("="*60)
    
    # Step 1: Build lightweight lookup
    ...
    # Step 2: Add columns quickly
    if add_columns_fast(args.csv_path, product_lookup):
        print(f"\n✅ Successfully processed {args.csv_path}!")
        print("The CSV file has been updated with new columns.")
    else:
        print(f"\n❌ Failed to process {args.csv_path}")
